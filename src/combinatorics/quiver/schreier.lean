...
abbreviation schreier_coset_graph (H : subgroup G) := sg (G ⧸ H) ι
...
{ obj := (mul_action.equiv_quotient_stabilizer G x₀).symm.to_fun,
  bij_obj := by { apply equiv.bijective, },
  map := by
  { rintros _ _ ⟨X,S⟩,
    dsimp [mul_action.equiv_quotient_stabilizer, mul_action.of_quotient_stabilizer],
